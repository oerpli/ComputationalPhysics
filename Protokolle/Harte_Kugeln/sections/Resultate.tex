...
Die Simulation wurde mit den in Tabelle~\ref{tab:tabPar} angeführten Parametern durchgeführt. Im weiteren Verlauf des Protokolls werden jeweils reduzierte Einheiten verwendet, als Längeneinheit wurde dabei der Durchmesser der Kugeln und als Gewichtseinheit das Gewicht der Kugeln verwendet.

\begin{table}[htb!]
\centering
\begin{tabular}{llll}\toprule
Zeitschritt        & $\Delta  t$ & $1\times 10^{-9}$            \\
Aufwärmzeit & --          & $10^8$& Zeitschritte \\
Simulationszeit      & --          & $10^9$& Zeitschritte\\\midrule
Initialisierung & -- & ??? \\\midrule
Anzahl der Kugeln & $N$         & $256$            \\
Dichte   & $\rho$      & $0.2$ -- $1.3$   \\
Radius der Kugeln & $r$         & $0.5$            \\
Masse der Kugeln   & $m$         & $1$              \\
Dimensionen & $D$         & $3$              \\
\bottomrule\end{tabular}
\caption{Für die Simulation verwendete Parameter}\label{tab:tabPar}

\end{table}

\subsection{Sampling}
\todo{Parameter und Details zu Durchführung (N, Aufwärmen, Anfangsbedingungen, reduzierte Einheiten, etc.) in diesen Absatz einbauen - irgendwer}
...
Abbildung \ref{fig:zustandsgleichung} zeigt die thermische Zustandsgleichung für das System aus harten Kugeln mit beiden Berechnungsmethoden. Zum Vergleich mit der Theorie ist auch die Zustandsgleichung nach Carnahan und Starling \cite{Carnahan1969} aufgetragen, welche eine gute einfache Näherung für den stabilen fluiden Bereich darstellt,
\begin{equation}
