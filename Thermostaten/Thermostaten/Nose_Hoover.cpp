...
void Nose_Hoover::Propagate(double step) {
	...
	for (int i = 0; i < poly.Get_length(); i++) {
		poly.Set_monomer_position(i, poly.Get_monomer_position(i) + (step - eta)*poly.Get_monomer_velocity(i) + step*step*0.5*(poly.Get_monomer_force(i)));
	}

	eta_new = eta + step*(poly.Get_ekin() - gkT*0.5) / q;

	for (int i = 0; i < poly.Get_length(); i++) {
		poly.Set_monomer_velocity(i, (1 - eta) * poly.Get_monomer_velocity(i) + step*0.5*(poly.Get_monomer_force(i)));
	}
