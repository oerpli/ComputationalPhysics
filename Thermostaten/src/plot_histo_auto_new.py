...
def plot_theory(kind , sim):
	if kind == "relPos": re = norm_func( 1./(sim.feder*sim.beta) )
                ...
                re = "(1./gamma(" + str(sim.p) + "/2))*((x/(2*" + str(sim.temp)+ "))**("+ str(sim.p) + "/2))"
                re = re + "*(1./x)*exp(-x/(2*" + str(sim.temp)+"))"		
...
class Simulation(object):
	...
	def plot_kind(self,s):
		...
		if s == "absPosition": return "absolut position [nm]"
		if s == "tempCol": return "temperature [K]"
		if s == "schwerPos" : return "center of mass [nm]"
