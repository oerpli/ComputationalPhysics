...
class Simulation(object):
	...
	def __init__(self,filename):
		...
		self.para = float( string_after( head , "couplTime" ) )
...
def set_titles(l_sim):
	...
	buf = l_sim[0].para
	flag = True
	for sim in l_sim: flag = flag and (buf == sim.para)
	if flag: title += " para=" + '%g' % (buf)
	else: 
		for sim in l_sim: sim.title += " para=" + '%g' % ( sim.para )
...
for kind in Simulation.all_kinds:
	...
	g( 'set term wxt ' + str( plot_count ) )
	g( 'set output' )
	g( plot_data )
	
	outF = "Histogramm_" + kind + ".png"
	g( 'set terminal pngcairo size 1024,768' )
	g( 'set output "' + outF + '"' )
	g( 'repl' )
