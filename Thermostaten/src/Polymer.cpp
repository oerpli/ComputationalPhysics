...
void Polymer::update_forces() {
        double force_buf = 0;
        auto mi=monomers.begin(), mj=monomers.begin();
        mj->force=0;

        for (mi=monomers.begin(), mj=mi, ++mj ; mi!=monomers.end() ; ++mi, ++mj) {
                if ( mj==monomers.end() ) mj = monomers.begin();
                else mj->force=0;
                
                force_buf = force(*mi - *mj);
                mi->force += force_buf;
                mj->force -= force_buf;
        }
}
...
double Polymer::update_ekin(){
	ekin = 0.0;
	for (auto& m : monomers) ekin += m.velocity*m.velocity;
